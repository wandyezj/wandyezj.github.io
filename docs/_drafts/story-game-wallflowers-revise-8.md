...
1. Give each player a blank index card and have them write down their _name_ in parentheses on one side, leaving the back blank.
2. Collect these index cards - this is the player deck
...
The facilitator will create both decks of cards, they will shuffle and draw for everyone as required.
...
In the first four rounds players take turns describing something at the party that wallflowers notice.
...
1. __Hide Wallflowers__
    - Collect the wallflower cards and shuffle them. Deal one card to each player face down. Do __NOT__ peak!
2. __Draw Opening Player__
    - Shuffle the player deck. Draw one card. The drawn player reads the opening line for the round and goes first.
